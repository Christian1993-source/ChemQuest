const STORAGE_KEY = "chemQuestHub.state.v2";
const LEGACY_STORAGE_KEY = "chemQuestHub.reviews.v1";
const GITHUB_SYNC_KEY = "chemQuestHub.githubSync.v1";
...
function inferTopic(prompt) {
  ...
  if (/(molar mass|mole|empirical formula|molecular formula|coefficient|balanced|conservation of mass|limiting reactant|percentage by mass|gram)/.test(text)) return "Stoichiometry";
...
function normalizeQuestion(rawQuestion, index = 0) {
  ...
  const topic = String(rawQuestion.topic || inferTopic(questionText)).trim() || "Chemistry";
...
function normalizeReview(rawReview, index = 0) {
  ...
  const title = String(rawReview.title || rawReview.unitName || `Chemistry Review ${index + 1}`).trim();
  ...
  return {
    ...
    title: title || `Chemistry Review ${index + 1}`,
...
function migrateLegacyReviews() {
  ...
  try {
    ...
    const fallbackReview = normalizeReview({
      ...
      title: rawOverview && rawOverview.unitName ? rawOverview.unitName : "Chemistry Review",
...
async function publishReviewsToGitHub(manual = false) {
  ...
  if (!state.sync.repo) {
    setSyncStatus("Open ChemQuest from your GitHub Pages link to publish live updates.", "warning");
    return false;
  }

  if (!state.sync.token.trim()) {
    setSyncStatus("Save your GitHub token first. Then ChemQuest can publish to students.", "warning");
    return false;
  }
  ...
  try {
    ...
    const response = await fetch(endpoint, {
      ...
      headers,
      body: JSON.stringify({
        message: `Publish ChemQuest reviews (${new Date().toLocaleString()})`,
...
function renderReviewGrid() {
  ...
  if (!state.reviews.length) {
    ...
    emptyCard.append(
      makeEl("h3", "", "No chemistry reviews yet"),
  ...
  state.reviews.forEach((review) => {
    ...
    const topics = makeEl(
      "div",
      "review-topics",
      review.topics.length ? review.topics.slice(0, 3).join(" • ") : "Chemistry practice review"
    );
    ...
    card.append(title, topics, stats, startBtn);
...
function renderQuestion() {
  ...
  topicPill.textContent = question.topic || "Chemistry";
  topicPill.classList.toggle("hidden", !question.topic);
...
function renderAdminList() {
  ...
  if (!state.reviews.length) {
    ...
    emptyRow.append(
      ...
      makeEl("div", "admin-meta", "Create your first chemistry review.")
    );
...
function parseReviewJsonInput(rawText) {
  ...
  if (!payload) {
    return { reviews: [], updatedAt: null, errors: ["The JSON structure is not valid for ChemQuest reviews."] };
  }
...
function exportReviews() {
  ...
  link.href = url;
  link.download = `chemquest-reviews-${new Date().toISOString().slice(0, 10)}.json`;
...
function clearLocalData() {
  const confirmed = window.confirm("Clear all ChemQuest reviews on this computer?");
  if (!confirmed) return;
